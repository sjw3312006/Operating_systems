...
void* List_remove(List* pList){
    ...
    if(pList -> current -> previous == NULL || pList -> current -> next == NULL){ //If the current pointer is before the start of the pList, or beyond the end of the pList
        return NULL; //don't change the list and return NULL
    }
    if(!(pList -> current -> previous == NULL)){
    ...
    if(pList -> current -> previous){
        pList -> current = pList -> current -> next;
    }
    else if(pList -> current -> next){
        pList -> current = pList -> current -> previous;
    }
    else{
        pList -> current = pList -> current -> next; //make the next item the current one
    }

    pList -> current -> next = NULL;
    pList -> current -> previous = NULL;
    pList -> count--; //since we took an item out of list
    return pList -> current -> data; //return current item
}
